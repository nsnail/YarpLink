namespace YarpLink.Domain.Contexts;
...
public sealed record ContextApp : DataAbstraction, IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        // 没有密码， 就要签名+时间戳
        if (Sign.NullOrEmpty()) {
            yield return new ValidationResult(Ln.Signature_is_missing, new[] { nameof(Sign) });
        }

        if (Math.Abs(DateTime.Now.TimeUnixUtc() - Timestamp) > _TS_OFFSET_SCOPE_SEC) {
            yield return new ValidationResult(Ln.Timestamp_Error, new[] { nameof(Timestamp) });
        }
    }
}